import { API_URL, ROUTES, showLoading, hideLoading, notify, IMAGE_MODEL } from "./config.js?v=20260918";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    blogForm.addEventListener("submit", async function (event) {
        ...
        // Auto-generate a deterministic AI cover when the user hasn't uploaded
        // or pasted an image URL and hasn't already hit "Surprise Image".
        // The AI reads the blog content (or honors a user-supplied prompt).
        let finalImageUrl = blogImageUrl || null;
        if (!finalImageUrl) {
            const customPrompt = aiCoverPrompt ? aiCoverPrompt.value.trim() : "";
            const autoCover = generateSmartCoverImage(blogTitle, blogBody, customPrompt || undefined);
            finalImageUrl = autoCover.url;
            if (imageInput) {
